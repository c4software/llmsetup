...
#
# Usage : ./setup-llm.sh --bench-agentic [modèle]
#
...
# lit avec les t/s. Journal : logs/bench-agentic.log (TSV, avec build).
# =============================================================================
...
cmd_bench_agentic() {
  ...
  info "bench-agentic '$preset' — 5 scénarios pi (tool calls) en direct sur $url"
  local sortie; sortie="$(mktemp)"
  MODEL="$preset" SERVER_URL="$url" docker compose -f "$SCRIPT_DIR/bench-agentic/docker-compose.yml" \
    run --rm --build pi 2>&1 | tee "$sortie" | grep -v $'^TSV\t' || true

  local dev
  dev="$(curl -s "$SPEC_TEST_URL/v1/models" 2>/dev/null \
    | python3 "$SCRIPT_DIR/py/spec_server_nmax.py" "$preset" --device 2>/dev/null || true)"
  # date modèle device build scénario verdict mur_s prompt_tok cache_tok gen_tok prefill_tps decode_tps
